 ...
 *   1. Local shadow `struct Enemy` / `struct dActor_c` (SpawnMegaCharParticles,
 *      SpawnCoin) replaced by include/Enemy.h's real classes -- two globally
 *      visible types named `Enemy` and `dActor_c` cannot survive in one TU.
...
/* -------------------------------------------------------------------------- */
/* Its legacy file built this from a local `struct Enemy : dActor_c` shadow with a
...
/* -------------------------------------------------------------------------- */
/* ROM ordinal 2 -- _ZN5Enemy22SpawnMegaCharParticlesER8dActor_cPc               */
...
void Enemy::SpawnMegaCharParticles(dActor_c &a, char *p)
{
    char *self = (char *)this;
    char *ap = (char *)&a;
    s16 dirvec[3];
