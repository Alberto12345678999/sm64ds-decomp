...
void _ZN12dEnemyBase_c12KillByAttackER5Actor(void *self, int a1, int a2, int a3);
}
...
extern "C" void func_ov004_020ae858(char *self)
{
    ...
    for (; i < 3; i++) {
        if (i == *(s16 *)(self + 0x4646)) {
            ...
            pos = tbl[i];
            _ZN12dEnemyBase_c12KillByAttackER5Actor((void *)data_ov006_021346bc, pos[0], pos[1], zero0);
        } else {
            s16 *pos = tbl[i];
            char *ep = (char *)extra[i];
            _ZN12dEnemyBase_c12KillByAttackER5Actor((void *)ep, pos[0], pos[1], zero1);
            _ZN12dEnemyBase_c12KillByAttackER5Actor((void *)data_ov006_021346bc, pos[0], pos[1], zero2);
        }
    }
}
