 ...
 *
 * The chain is ActorBase -> dBase_c -> dActor_c. dActor_c is NOT a direct child
 * of ActorBase: dActor_c::dActor_c calls ActorBase::ActorBase, stores dBase_c's
 ...
 *
 * LAYOUT. ActorBase occupies 0x00..0x4f, so dActor_c's own fields begin at 0x50.
...
struct dActor_c : dBase_c {
    ...
    /* 0x0a4 and 0x0ac were padding "likely the same physics block; unproven".
       ...
       both as s32 and its sources read them, so once `Enemy : dActor_c` they have
    ...
    /* THE COUNTERPART OF ActorBase::operator new, AND WHAT LETS A REAL `~Class()`
       ...
       On ActorBase, every dActor_c-derived D0 emits an out-of-line call instead and
