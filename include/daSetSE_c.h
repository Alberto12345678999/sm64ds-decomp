...
struct daSetSE_c : dActor_c {
    ...
    /* INLINE, AND THAT IS WHAT LETS THIS CLASS OWN A TRANSLATION UNIT. Declared
       ...
       THE BODY IS EMPTY, and the ROM agrees: D1 is 0x24 bytes, one vptr store
       plus the tail into ~dActor_c. `daSetSE_c : dActor_c` is a one-level
       chain, so two vptr stores come out of it, and no member of either class
