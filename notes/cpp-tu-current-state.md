...
| Physical production source files | 10917 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10917 |
| Tracked `.c` files | 6183 |
| Tracked `.cpp` files | 4734 |
...
| Source-built (`complete`) source files | 10690 |
| Complete sources owning more than one function | 49 |
| Source-built records inside multi-function sources | 451 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 50 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 44 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 48, `text-verified` 47.
