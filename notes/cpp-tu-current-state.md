...
| Strict CONVERTED source functions | 2592 / 11315 (22.91%) |
| Physical production source files | 10631 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10631 |
| Tracked `.c` files | 6070 |
| Tracked `.cpp` files | 4561 |
...
| Source-built (`complete`) source files | 10404 |
| Complete sources owning more than one function | 76 |
| Source-built records inside multi-function sources | 765 |
...
| Functions named by manifest entries | 1415 |
| Unique functions named by the manifest | 1415 |
| Modules represented | 34 |
| Manifest shadow sources present in git | 108 |
| Entries licensing non-text sections | 14 |
| Entries actually production-enrolled at `promoted_source` | 71 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 75, `text-verified` 30.
