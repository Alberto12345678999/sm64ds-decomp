...
| Physical production source files | 10906 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10906 |
| Tracked `.c` files | 6180 |
| Tracked `.cpp` files | 4726 |
...
| Source-built (`complete`) source files | 10679 |
| Complete sources owning more than one function | 51 |
| Source-built records inside multi-function sources | 464 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 48 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 46 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 50, `text-verified` 45.
