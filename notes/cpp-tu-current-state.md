...
| Strict CONVERTED source functions | 2550 / 11313 (22.54%) |
| Physical production source files | 11243 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 11243 |
| Tracked `.c` files | 6373 |
| Tracked `.cpp` files | 4870 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3559 |
| Genuinely migrated C++ symbol files | 3485 |
...
| Source-built (`complete`) source files | 11016 |
| Complete sources owning more than one function | 8 |
| Source-built records inside multi-function sources | 83 |
...
Current complete multi-function owners: `src/actors/ActorBase_SceneNode.cpp` (2 function-symbol records), `src/actors/ActorDerived.cpp` (5 function-symbol records), `src/actors/daKpFr_c.cpp` (21 function-symbol records), `src/actors/daKrpa_c.cpp` (25 function-symbol records), `src/actors/daObjAbuku_c.cpp` (7 function-symbol records), `src/actors/daObjPathLift_c.cpp` (8 function-symbol records), `src/actors/daObjPushblock_c.cpp` (9 function-symbol records), `src/func_01ff97d8.c` (6 function-symbol records).
...
| Manifest entries | 90 |
| Functions named by manifest entries | 1117 |
| Unique functions named by the manifest | 1117 |
| Modules represented | 27 |
| Manifest shadow sources present in git | 90 |
| Entries licensing non-text sections | 2 |
| Entries actually production-enrolled at `promoted_source` | 7 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 4, `promoted` 7, `text-verified` 79.
