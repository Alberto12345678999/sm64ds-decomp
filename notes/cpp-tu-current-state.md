...
| Strict CONVERTED source functions | 2528 / 11314 (22.34%) |
| Physical production source files | 11192 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 11192 |
| Tracked `.c` files | 6350 |
| Tracked `.cpp` files | 4842 |
...
| Source-built (`complete`) source files | 10965 |
| Complete sources owning more than one function | 19 |
| Source-built records inside multi-function sources | 146 |
...
| Entries actually production-enrolled at `promoted_source` | 18 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 18, `text-verified` 69.
