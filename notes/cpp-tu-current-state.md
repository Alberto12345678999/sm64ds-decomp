...
| Physical production source files | 10912 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10912 |
| Tracked `.c` files | 6182 |
...
| Source-built (`complete`) source files | 10685 |
| Complete sources owning more than one function | 50 |
| Source-built records inside multi-function sources | 457 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 49 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 45 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 49, `text-verified` 46.
