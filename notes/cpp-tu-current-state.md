...
| Strict CONVERTED source functions | 2591 / 11315 (22.90%) |
| Physical production source files | 10676 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10676 |
| Tracked `.c` files | 6098 |
| Tracked `.cpp` files | 4578 |
...
| Source-built (`complete`) source files | 10449 |
| Complete sources owning more than one function | 74 |
| Source-built records inside multi-function sources | 718 |
...
| Functions named by manifest entries | 1368 |
| Unique functions named by the manifest | 1368 |
| Modules represented | 33 |
| Manifest shadow sources present in git | 106 |
| Entries licensing non-text sections | 14 |
| Entries actually production-enrolled at `promoted_source` | 69 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 73, `text-verified` 30.
