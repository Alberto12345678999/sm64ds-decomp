...
| Strict CONVERTED source functions | 2551 / 11314 (22.55%) |
| Physical production source files | 11245 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 11245 |
| Tracked `.c` files | 6372 |
| Tracked `.cpp` files | 4873 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3560 |
...
| Source-built (`complete`) source files | 11018 |
| Complete sources owning more than one function | 8 |
| Source-built records inside multi-function sources | 82 |
...
Current complete multi-function owners: `src/actors/ActorBase_SceneNode.cpp` (2 function-symbol records), `src/actors/ActorDerived.cpp` (5 function-symbol records), `src/actors/daKpFr_c.cpp` (21 function-symbol records), `src/actors/daKrpa_c.cpp` (25 function-symbol records), `src/actors/daObjAbuku_c.cpp` (7 function-symbol records), `src/actors/daObjCtMecha05_c.cpp` (8 function-symbol records), `src/actors/daObjPathLift_c.cpp` (8 function-symbol records), `src/func_01ff97d8.c` (6 function-symbol records).
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 82 |
| Manifest entries | 89 |
| Functions named by manifest entries | 1107 |
| Unique functions named by the manifest | 1107 |
...
Manifest statuses: `link-verified` 4, `promoted` 7, `text-verified` 78.
