...
| Physical production source files | 10855 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10855 |
| Tracked `.c` files | 6169 |
| Tracked `.cpp` files | 4686 |
...
| Source-built (`complete`) source files | 10627 |
| Complete sources owning more than one function | 58 |
| Source-built records inside multi-function sources | 523 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 41 |
| Manifest entries | 98 |
...
| Entries licensing non-text sections | 9 |
| Entries actually production-enrolled at `promoted_source` | 53 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 57, `text-verified` 38.
