...
| Physical production source files | 10899 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 10899 |
| Tracked `.c` files | 6179 |
| Tracked `.cpp` files | 4720 |
...
| Source-built (`complete`) source files | 10672 |
| Complete sources owning more than one function | 52 |
| Source-built records inside multi-function sources | 472 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 47 |
| Manifest entries | 98 |
...
| Entries actually production-enrolled at `promoted_source` | 47 |
| Existing promotion paths that disagree with delinks | 4 |

Manifest statuses: `link-verified` 3, `promoted` 51, `text-verified` 44.
