...
| Strict CONVERTED source functions | 2538 / 11314 (22.43%) |
| Physical production source files | 11207 |
...
| Measure | Live value |
| --- | ---: |
| Tracked production source files | 11207 |
| Tracked `.c` files | 6354 |
| Tracked `.cpp` files | 4853 |
| `.cpp` files missing first-line `//cpp` | 0 |
| Mangled-symbol source files | 3569 |
| Genuinely migrated C++ symbol files | 3495 |
...
| Source-built (`complete`) source files | 10980 |
| Complete sources owning more than one function | 17 |
| Source-built records inside multi-function sources | 129 |
...
| Measure | Live value |
| --- | ---: |
| Tracked `src_tu/` source files | 74 |
| Manifest entries | 90 |
...
| Entries actually production-enrolled at `promoted_source` | 16 |
| Existing promotion paths that disagree with delinks | 0 |

Manifest statuses: `link-verified` 3, `promoted` 16, `text-verified` 71.
