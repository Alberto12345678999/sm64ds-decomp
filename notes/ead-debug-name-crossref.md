...
([daObjGuragura_c.h](../include/daObjGuragura_c.h), ov002, `_ZTI15daObjGuragura_c`) with its own header and
...
|---|---|---|---|---|
| 14 | CANNON_SHUTTER | Former `CANNON_HATCH` gloss; current code uses ROM-proven `daObjCannonShutter_c` | [ov002/symbols.txt](../config/arm9/overlays/ov002/symbols.txt) | **Resolved.** `ov002/symbols.txt` has `_ZTS20daObjCannonShutter_c`, and current main already adopted that exact RTTI spelling. The independent ROM debug string confirms "Shutter," so this change updates only the remaining community gloss rather than replacing the stronger class identity. |
