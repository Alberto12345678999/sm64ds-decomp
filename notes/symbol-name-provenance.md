...
| `symbols/verified.tsv` | the **import mirror** (address → name, flat, no module column). Rows are restamped to the config spelling as corrections land; import-time spellings live in git history. Never a join key for names — see "Two records, one join key" |
| `symbols/record_divergences.tsv` | the banked enumeration of every config/mirror disagreement, regenerated by `tools/symrecords.py --report` |
