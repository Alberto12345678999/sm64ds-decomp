...
**Written 2026-08-07. The work described here is on `main`.** Everything below is
checkable; every number appears with the command that produces it. Where an earlier
answer was wrong, it is recorded rather than quietly fixed.

It arrived by an unfortunate route, which matters only because the intermediate state
was briefly wrong and someone may have pulled it. **#1207** committed the tools from an
uncommitted working tree and got an arbitrary mid-flight slice: it deleted
`include/daObjSwdoor_c.h` while shipping a `tools/rtti_vtables.py` that still had the
over-read, so `--emit-headers` on that commit recreated the header with four fields
nothing supports. **#1210** then carried the finished versions, and `main` has been
consistent since. If you are looking at anything between those two commits, regenerate
before trusting it.
...
`docs/class-reference.html` is generated and committed, so it is a snapshot and drifts
as `src/` and `config/` move. Regenerating it against `main`'s `config/` was checked and
produced byte-identical output, so the *named* figures are main-native; only the
**enrolled** column depends on `build/eligible-names.txt`, which is a build artifact and
therefore reflects whatever tree last produced it. Re-run `tools/rtti_reference.py` after
any batch that enrolls functions. `--check` refuses to emit on stale inputs and warns if
any slot resolves to no symbol.
